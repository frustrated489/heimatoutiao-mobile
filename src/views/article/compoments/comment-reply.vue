  ...
  methods: {
    async onLoad () {
      ...
      const res = await getComments({
        ...
        source: this.comment.com_id.toString(), // 源id，对文章id或评论id
    ...
    async onAddComment () {
      ...
      const res = await addComment({
        ...
        art_id: this.$route.params.articleId // 文章id，对评论内容发表回复时，需要传递此参数，表面所属文章id。对文章进行评论，不要传递此参数
      })
      // 将发布的最新评论展示到列表顶部
